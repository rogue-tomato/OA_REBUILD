...
function closeLauncherApp() {
    if (window.OathswornAndroid && typeof window.OathswornAndroid.closeApp === 'function') {
        window.OathswornAndroid.closeApp();
        return;
    }

    fetch('/__quit', { method: 'POST', cache: 'no-store' })
        .then(response => {
            if (!response.ok) throw new Error('Launcher quit endpoint unavailable');
            window.close();
            setTimeout(() => alert('If this window did not close, press Alt+F4.'), 250);
        })
        .catch(() => {
            window.close();
            setTimeout(() => alert('Close the fullscreen window with Alt+F4.'), 250);
        });
}

if (location.protocol === 'http:' || location.protocol === 'https:') {
    setInterval(() => {
        fetch('/__ping', { method: 'POST', cache: 'no-store' }).catch(() => {});
    }, 3000);
}
...
$('#btn-launcher-close').on('click', closeLauncherApp);
